...
void  tft_get_info() noexcept
{
      ...
      uint8_t p3;
      uint8_t p4;
      tft_recv(0x04, p1, p2, p3, p4);
      nop();
}

uint32_t tft_get_status() noexcept
{
      uint8_t p1;
      uint8_t p2;
      uint8_t p3;
      ...
      l_result = (p2 << 24) | (p3 << 16) | (p4 << 8) | p5;
      return l_result;
}

uint32_t tft_get_scanline() noexcept
{
      uint8_t p1;
      uint8_t p2;
      uint8_t p3;
...
uint32_t tft_get_power_mode() noexcept
{
      uint8_t p1;
      uint8_t p2;
      tft_recv(0x0a, p1, p2);
...
void  tft_core1_boot() noexcept
{
      ...
      uint64_t        l_delay = 1000000 / 70 / g_screen_height;
...
void  test_stdlib() noexcept
{
      char* data = reinterpret_cast<char*>(malloc(64));
      if(data) {
          free(data);
      }
}

void  test_solid(int cycles) noexcept
{
...
void  test_scanner(int cycles) noexcept
{
      int16_t l_py = 0;
      int16_t l_dir = 1;
      int16_t l_colour = rand() & 65536;
      for(int index = 0; index < cycles; index++) {
          auto l_line = g_screen_ptr + g_screen_width * l_py;
          for(int l_px = 0; l_px < g_screen_width; l_px++) {
              l_line[l_px] = l_colour;
          }
          l_py += l_dir;
          if(l_dir > 0) {
              if(l_py == g_screen_height) {
                  l_colour = rand() % 65536;
                  l_py = g_screen_height - 1;
                  l_dir = -1;
              }
          } else
          if(l_dir < 0) {
